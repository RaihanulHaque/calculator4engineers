...
def primeFactors(n):
    ...
    while n > 1:
        ...
        if n % c == 0:
            ...
            c = c + 1
    left.append(1)
    return primef, left, str_primef[:-2], multi_primef[:-2], expo_primef[:-2]
...
def isPrime_or_Composite(n):
    primef, left, str_primef, multi_primef, expo_primef = primeFactors(n)
    if len(primef) > 1:
...
content = content + wp_table(f"<tr><td>Prime Factors</td><td>{str_primef}</td></tr><tr><td>Product of Prime Factor</td><td>{multi_primef}</td></tr><tr><td>Exponential Form</td><td>{expo_primef}</td></tr><tr><td>Total Number of Factors</td><td>{len(primef)}</td></tr><tr><td>Largest Prime Factor</td><td>{max(primef)}</td></tr><tr><td>Smallest Prime Factor</td><td>{min(primef)}</td></tr><tr><td>Closest Prime Numbers</td><td>{nearestPrime(n)}</td></tr>")
...
content = content + \
    wp_paragraph("N = p<sub>f1</sub><sup>a1</sup> +&nbsp; p<sub>f2</sub><sup>a2</sup> +&nbsp; &nbsp; p<sub>f3</sub><sup>a3</sup> + ... ... +&nbsp; p<sub>fn</sub><sup>an</sup>")
